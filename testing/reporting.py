...
def create_config_label(row):
    ...
    if "Improved" in algo_name:
        splitter = (
            str(row.get("splitter", "NA"))
            .replace("SplitLongestSide", "Longest")
            .replace("SplitGradient", "Gradient")
        )
...
def generate_per_problem_plots(df):
    ...
    for problem in problems:
        ...
        problem_df["config_label"] = pd.Categorical(
            problem_df["config_label"], categories=ALGO_SORT_ORDER, ordered=True
        )
        ...
        output_path_svg = (
            OUTPUT_DIR / f"full_comparison_{problem.replace(' ', '_')}.svg"
        )
...
def generate_aggregate_plot(df):
    ...
    """
    plot_df = (
        df[df["config_label"] != "Baseline(dReal)"]
    ...
    plot_df["config_label"] = pd.Categorical(
        ...
        categories=[c for c in ALGO_SORT_ORDER if c != "Baseline(dReal)"],
        ordered=True,
    )
...
def generate_aggregate_bound_plot(df):
    ...
    baseline_bounds_map = (
        df[df["config_label"] == "Baseline(dReal)"]
        .set_index("problem")["bound"]
        .to_dict()
    )
    plot_df = df[df["config_label"] != "Baseline(dReal)"].copy()
    plot_df["baseline_bound"] = plot_df["problem"].map(baseline_bounds_map)
    # Calculate the normalized difference with careful handling of division by zero
    plot_df = plot_df.dropna(subset=["bound", "baseline_bound"])

    # Initialize the new column
    plot_df["normalized_bound_difference"] = np.nan

    # Case 1: baseline_bound is not zero
    mask_nonzero = plot_df["baseline_bound"] != 0
    plot_df.loc[mask_nonzero, "normalized_bound_difference"] = (
        plot_df.loc[mask_nonzero, "bound"] - plot_df.loc[mask_nonzero, "baseline_bound"]
    ) / plot_df.loc[mask_nonzero, "baseline_bound"]

    # Case 2: baseline_bound is zero, and the algorithm's bound is also zero
    mask_zero_and_zero = (plot_df["baseline_bound"] == 0) & (plot_df["bound"] == 0)
    plot_df.loc[mask_zero_and_zero, "normalized_bound_difference"] = 0

    # Case 3 (implicit): baseline_bound is 0 and bound is not, will remain NaN and be dropped
    plot_df = plot_df.dropna(subset=["normalized_bound_difference"])

    # Per user request, filter out algorithms with high variance to improve plot readability
    removed_for_clarity = []  # ["Longest-Affine", "Gradient-Affine", "Gradient-Bernstein"]
    ...
    plot_df["config_label"] = pd.Categorical(
        ...
        categories=[
            c
            for c in ALGO_SORT_ORDER
            if c not in ["Baseline(dReal)"] + removed_for_clarity
        ],
        ordered=True,
    )
    plot_df = plot_df.sort_values("config_label")
    ...
    data_to_plot = [
        plot_df[plot_df["config_label"] == config]["normalized_bound_difference"]
        for config in configs
    ]
    ...
    ax.set_ylabel("Normalized Bound Difference ((bound - baseline) / baseline)")
    ...
    fig.text(
        0.02,
        0.02,
        f"Removed for clarity: {', '.join(removed_for_clarity)}",
        ha="left",
        va="bottom",
        fontsize=10,
        color="gray",
...
def main():
    ...
    df_combined = df_combined[
        df_combined["algorithm"] != "GlobalMinBranchAndBound"
    ].copy()
    ...
    df_combined["baseline_runtime_for_problem"] = df_combined["problem"].map(
        baseline_runtimes_map
    )

    df_combined["normalized_runtime"] = np.nan
    mask = (
        (df_combined["config_label"] != "Baseline(dReal)")
        & df_combined["runtime"].notna()
        & df_combined["baseline_runtime_for_problem"].notna()
    )
    df_combined.loc[mask, "normalized_runtime"] = (
        df_combined.loc[mask, "runtime"]
        / df_combined.loc[mask, "baseline_runtime_for_problem"]
    )
